defmodule WerewolfApiWeb.GameChannelTest do
  ...
  describe "cancel_action event" do
    test "cancel_action responds with ok on success" do
      user = insert(:user)
      other_id = user.id + 1
      game = insert(:game)
      game_id = game.id
      ...
      ref = push(game_socket, "cancel_action", %{target: other_id, action_type: "vote"})

      assert_broadcast("game_state_update", %{
        id: ^game_id,
        players: %{^user_id => %{actions: %{2 => %{}}}}
      })

      assert_reply(ref, :ok)
    end

    test "cancel_action responds with error on failure" do
      user = insert(:user)
      other_id = user.id + 1
      game = insert(:game)
      game_id = game.id
      ...
      state = %{
        game: %Werewolf.Game{
          ...
          phase_length: :day,
          phases: 2,
          options: %Werewolf.Options{allow_action_changes: false},
          players: %{
            user.id => %Werewolf.Player{
              actions: %{},
              alive: true,
      ...
      ref = push(game_socket, "cancel_action", %{action_type: "vote"})
    ...
    test "action responds with error on failure" do
      ...
      state = %{
        game: %Werewolf.Game{
          ...
          phase_length: :day,
          phases: 2,
          options: %Werewolf.Options{allow_claim_role: false},
          players: %{
            user.id => %Werewolf.Player{
              actions: %{},
              alive: true,
  ...
  describe "end_phase event" do
    ...
    test "end_phase responds with error on failure" do
      user = insert(:user)
      other_id = user.id + 1
      game = insert(:game)
      game_id = game.id
      ...
      state = %{
        game: %Werewolf.Game{
          ...
          id: game_id,
          phase_length: :day,
          options: %Werewolf.Options{allow_host_end_phase: false},
          phases: 2,
          players: %{
            user.id => %Werewolf.Player{
              actions: %{},
  ...
  describe "end_game event" do
    test "end_game responds with ok on success" do
      user = insert(:user)
      game = insert(:game)
      game_id = game.id
      user_id = user.id
      ...
      state = %{
        game: %Werewolf.Game{
          ...
          phase_length: :day,
          phases: 2,
          options: %Werewolf.Options{allow_host_end_game: true},
          players: %{
            user.id => %Werewolf.Player{
              actions: %{},
              alive: true,
    ...
    test "end_game responds with error on failure" do
      user = insert(:user)
      other_id = user.id + 1
      game = insert(:game)
      game_id = game.id
      ...
      state = %{
        game: %Werewolf.Game{
          ...
          phase_length: :day,
          phases: 2,
          options: %Werewolf.Options{allow_host_end_game: false},
          players: %{
            user.id => %Werewolf.Player{
              actions: %{},
              alive: true,
